<ul class="list-unstyled fw-normal pb-2 small" style="padding-left:1rem;">
    @foreach($items as $item)
    <li>
        <div class="d-flex w-100 gap-2 align-items-center">
            <a href="javascript:void(0);"
                        class="bd-links-link d-inline-block rounded"
                        wire:click="goToPage($event.shiftKey,'{{$item['id']}}')">
                        @if(isset($item['name']))
                        {{$item['name']}}
                        @endif
            </a>

            {{-- @if(isset($item['link']))
            <a href="{{$item['link']}}" class="bd-links-link d-inline-block rounded">
                {{$item['name']}}
            </a>
            @else
            <a href="javascript:void(0);" class="bd-links-link d-inline-block rounded">
                {{$item['name']}}
            </a>
            @endif --}}

            {{-- <span wire:click="edit('{{$item['id']}}')">
                <svg xmlns="http://www.w3.org/2000/svg" width="8" height="8" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                    <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                    <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
                </svg>
            </span> --}}
        </div>

        {{-- 재귀호출 --}}
        @if(isset($item['items']))
            @include($viewListItem,['items' => $item['items']])
        @endif

    </li>
    @endforeach
</ul>
